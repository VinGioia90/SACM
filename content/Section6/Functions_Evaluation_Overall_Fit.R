...
# New
form_mvnchol <- function(d){
  foo <- make_formula(as.formula(paste(paste(paste0("y_", 1 : d),  collapse = " | "), "~ s(sx0) | s(sx0) | s(sx0)")))
  return(foo)
}
...
sim_est_efs_bfgs_bamlss <- function(nobs, dgrid,  nrun, ncores, param1 = "mcd", expl_mean, expl_Theta,
