  ...
  sim1 <- mclapply(1 : nrun, function(ii){
    dss <- lapply(1 : length(dgrid), function(jj){
      ...
      if(save.gam == "TRUE"){
        ...
        out$lpi_pred <- predict(fit)  ##!! Decide whether leaving the lpi_pred (maybe better) or only return mean and varcov
        fit$family$put_cflag(FALSE) # Return the covariances instead of the correlations
        out$vcov_pred <- predict(fit, type = "response") ##!!! see comment above
      }
      return(out)
    })
    return(list("gen" = dss))
