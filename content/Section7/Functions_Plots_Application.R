...
res_perf <- function(obj, dgrid, d, data_train, data_test){
...
##########################################################################
get_plots <- function(obj, name_eff, d, eff_grid){
  #removed first (full) and last (static)
  res_plot <- lapply(2 : (length(obj$fit) - 1), function(x) get_eff_idx(obj = obj$fit[[x]], name_eff = name_eff, d = d))
  ...
  for(j in 1 : length(res_plot)){
    pl[[j]] <-ggplot(res_plot[[j]], aes(x = jit_col, y = jit_row, shape = name_eff))+
      ...
      theme_bw() +
      labs(shape = "Effect") +
      labs(title=paste("Covariance matrix model with", eff_grid[j], "effects"))+
