...
datagen <- function(d = 2, n = 1000, pint = 0, seed = 13, param = NULL){
  ...
  fvcov <- function(x1, x2, c1, c2, c3, c4, c5, c6, beta0c, ind) {
    beta0c + ind * (c1 * sin(2 * pi * (x1 + c2)) + c3 * cos(2 * pi * (x1 + c2)) +
                      c4 * sin(2 * pi * (x2 + c5)) + c6 * cos(2 * pi * (x2 + c5)))
  }
  
  c1 <- runif(d * (d + 1)/2, -0.5, 0.5)
  c2 <- runif(d * (d + 1)/2, -0.5, 0.5)
  c3 <- runif(d * (d + 1)/2, -0.25, 0.25)
  c4 <- runif(d * (d + 1)/2, -1, 1)
  c5 <- runif(d * (d + 1)/2, -1, 1)
  c6 <- runif(d * (d + 1)/2, -0.5, 0.5)
  ...
  for(i in 1:n){
    ...
    Theta[[i]][1,1] <- fvcov(sx0[i], sx1[i], c1[1], c2[1], c3[1], c4[1], c5[1], c6[1], beta0c[1],  intercepts[1])
    count <- d + 1
    for(j in 2:d){
      Theta[[i]][j,j] <- fvcov(sx0[i], sx1[i], c1[j], c2[j], c3[j], c4[j], c5[j], c6[j], beta0c[j],  intercepts[j])
      for(k in 1 : (j - 1)){
        Theta[[i]][j,k] <- Theta[[i]][k,j] <-fvcov(sx0[i], sx1[i], c1[count], c2[count], c3[count], c4[count], c5[count], c6[count], beta0c[count],  intercepts[count])
        count <- count + 1
      }
    }
    if ( param ==  "mcd" ) {
  ...
  return(list(data = cbind(data, sx0, sx1), Sigma = Sigma, Theta=Theta, mu=mu, idxint = intercepts))
}
...
mformula <- function(d = 2, expl_mean, expl_Theta, idxcov_int = rep(1, d*(d+1)/2)){
  ...
  meanv_l <- paste0("y_", d, "~ s(", expl_mean, ")")
  ...
  if( lnoint > 0){
    ...
    covx_foo_l <- paste0( labelTh_x[lnoint], "~ s(", expl_Theta[1], ") + s(", expl_Theta[2],")")
...
old <- Sys.time()
 sim_mcdG_mcdF <- sim_est_efs(nobs, dgrid, nrun, ncores, param1="mcd", param2="mcd", save.gam=sg,
                              expl_mean = "sx0", expl_Theta=c("sx0", "sx1"))
new <- Sys.time() - old
print(new)
#Time difference of 9.898622 mins
...
new <- Sys.time() - old
print(new)
#Time difference of 14.37273 mins
...
new <- Sys.time() - old
print(new)
#Time difference of 14.56749 mins
...
new <- Sys.time() - old
print(new)
#Time difference of 10.05243 mins
